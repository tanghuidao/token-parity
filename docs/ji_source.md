...
> **落地附注（2026-09-01，P0）**：本表已于同日按 §7.3 预留的"新增列"路线落地为置信带——`parity_index.py` 为每个篮子模型配置本表三档 jᵢ，主序列新增 `R_A_low/high`、`Lambda_low/high` 四列（逐模型替换 jᵢ 重新聚合，非 j̄ 整体缩放），主列点值与历史数值零影响，历史行带列留空不回填。V5 裁定（主口径维持点值）继续有效：中档 = 主列点值，三档仅用于附加区间列。详见 changelog 与 README「Λ 置信带（jᵢ 三档）」小节。
