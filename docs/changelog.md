...
| 2026-09-01 | P1 第二价格源升级为稳健性对照 | LiteLLM 牌价列由"仅验证"升级为对照口径：主序列新增 R_A_alt / Lambda_alt / alt_deviation 三列（第二源牌价、同一权重同一 jᵢ 中档平行计算；alt_deviation 为小数，0.02 即 +2%）。覆盖完整（4/4）才计算，否则置空并告警；偏离超 ±15% 触发哨兵（历史观测区间 +1.3% 至 +9.8%）。主列继续只用 OpenRouter 市场价，**不自动切换源**（源切换须人工决策）。缓解 L6（单一价格源） |
| 2026-09-01 | O1a 历史回填·挖矿侧上线 | 新增一次性脚本 `backfill_rm.py` + 独立月频序列 `docs/rm_history.csv`（2010-07 → 今，194 行，source=backfill_v1）：R_M 与 ε_BTC 回填至 GPU 时代。数据源 blockchain.info charts（价格/算力/手续费，日频，免费无 key）；队列能效历史表为政策内参数（初稿 v0.1，机型代表法，溯源文档 `docs/efficiency_history_source.md` 草稿待核验 E1–E5）。每行带 quality 分层（low=GPU/FPGA 时代 / medium=ASIC 时代）。**独立序列不进主序列、不进大屏、不改 parity_index.py**；重叠期交叉验证通过（回填 vs 主序列 R_M 偏差 +1.43%，阈值 ±5%）。O1b（RA 侧历史前沿篮子）另立项 |
