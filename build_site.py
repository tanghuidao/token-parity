...
def build(rows):
    ...
    s = {k: [float(r[k]) for r in rows] for k in ("R_M", "R_A", "Lambda", "Omega")}

    rm, ra = float(latest["R_M"]), float(latest["R_A"])
    lam, om = float(latest["Lambda"]), float(latest["Omega"])
...
<div class="cards">
  <div class="card"><div class="k">R_M · 挖矿每度电毛收入</div>
    <div class="v copper">${rm:.4f}</div><div class="d">美元/kWh，由链上数据自算 hashprice ÷ 队列能耗</div></div>
  ...
  <div class="card"><div class="k">Λ · 能量套利比</div>
    <div class="v graphite">{lam:.1f}</div><div class="d">R_A / R_M，毛收入口径</div></div>
  <div class="card"><div class="k">Ω · 平价偏离指数</div>
    <div class="v oxblood">{om:.3f}</div><div class="d">ln(焦耳平价汇率 / 市场隐含汇率)，防御性 vs 生产性耗散的定价缺口</div></div>
</div>

<section class="charts">
  <div class="chart-grid" id="jscharts">
    <div class="chartbox"><h3 style="color:var(--copper)">R_M（$/kWh）</h3><canvas id="c_rm"></canvas></div>
    <div class="chartbox"><h3 style="color:var(--cobalt)">R_A（$/kWh）</h3><canvas id="c_ra"></canvas></div>
    <div class="chartbox"><h3 style="color:var(--graphite)">Λ 能量套利比</h3><canvas id="c_l"></canvas></div>
    <div class="chartbox"><h3 style="color:var(--oxblood)">Ω 平价偏离指数</h3><canvas id="c_o"></canvas></div>
  </div>
...
<details>
  ...
  <p style="margin-top:10px">挖矿侧：<code>hashprice = 近144块总奖励(BTC) × BTC价格 ÷ 全网算力(PH/s)</code>，再除以每 PH/s 的日耗电（队列能效 × 24h）得 R_M。单枚 BTC 体现能 ε = 全网日耗能 ÷ 日产出。</p>
  <p>推理侧：模型篮子按用量加权；质量折算采用"升贴水法"（仿原油基准分级）——以基准模型价格为 1，其他模型质量权重 = 其价格 ÷ 基准价，质量调整落在数量侧。</p>
  <p>Ω 的含义：比特币的能量用于"设防"（不可伪造的耗费），AI token 的能量用于"生产"。Ω 度量市场为这两种耗散支付的每焦耳价格之差，其<b>时间变化</b>比水平值更可信。</p>
  <ul>
    <li>队列能效与单 token 能耗为手动参数（参考 CBECI / Epoch AI），是主要误差源；</li>
    <li>Λ 为毛收入口径，非利润口径——推理侧成本大头是 GPU 折旧而非电费；</li>
    <li>篮子成分变更采用环比拼接，换基日期记录于仓库提交历史。</li>
  </ul>
</details>
